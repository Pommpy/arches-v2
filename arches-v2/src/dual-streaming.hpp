...
#include "arches/units/dual-streaming/unit-sfu.hpp"
#include "arches/units/dual-streaming/unit-tp.hpp"
...
inline void run_sim_trax()
{
	...
	std::vector<Units::DualStreaming::UnitSFU*>sfus;
	std::vector<std::vector<Units::DualStreaming::UnitSFU*>>sfus_tables;
	...
	for(uint k = 0; k < num_l2; ++k)
	{
		...
		for(uint j = 0; j < num_tms_per_l2; ++j)
		{
			...
			Units::DualStreaming::UnitSFU** sfu_table = sfus_tables.back().data();
			tp_config.mem_higher = l1;
			tp_config.sfu_table = sfu_table;

			sfus.push_back(new Units::DualStreaming::UnitSFU(2, 8, num_tps_per_tm, &simulator));
			...
			sfus.push_back(new Units::DualStreaming::UnitSFU(1, 2, num_tps_per_tm, &simulator));
			sfu_table[static_cast<uint>(ISA::RISCV::Type::IMUL)] = sfus.back();
			sfu_table[static_cast<uint>(ISA::RISCV::Type::IDIV)] = sfus.back();
			simulator.register_unit(sfus.back());

			sfus.push_back(new Units::DualStreaming::UnitSFU(20, 1, num_tps_per_tm, &simulator));
