...
public:
	...
	BusGroup<Request> request_bus;
	BusGroup<Request> return_bus;
...
private:
	...
	void clock_rise() override
	{
		...
		for(uint i = 0; i < issue_width; ++i)
		{
			...
			Request request_item = request_bus.get_bus_data(request_index);
	...
	void clock_fall() override
	{
		while(!_return_queue.empty() && _return_queue.top().return_cycle <= current_cycle)
		{
			...
			return_bus.set_bus_data(return_item.request, return_item.index);
