...
inline static bool intersect(const TesselationTree1Pointers& tes_tree, const float recip_max_error, const uint32_t patch_index, const Ray& ray, const glm::vec3& inv_d, Hit& hit)
{
	const TesselationTree1::Header header = tes_tree.headers[patch_index];
	...
	do
	{
		...
		tri_stack[lod].new_center_tri = get_transformed_triangle(tri_stack[lod].tri, decompact(triangles[absoulte_node_index]), edge_states);
		...
		else //interior node
		{
			...
			for(uint32_t i = 0; i < 4; ++i)
			{
				TesselationTree1::Node node = nodes[absolute_first_child_index + i];
...
inline bool intersect(const TesselationTree1Pointers tes_tree, const float recip_max_error, const Ray& ray, Hit& hit)
{
