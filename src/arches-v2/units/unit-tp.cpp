...
void UnitTP::data_stall_switch_thread() {
...
void UnitTP::resource_stall_switch_thread() {
	uint new_thread_id = thread_arbiter.get_index();
	if (new_thread_id == ~0) return;
	thread_arbiter.remove(new_thread_id);
	thread_arbiter.add(current_thread_id);
...
void UnitTP::clock_rise()
{
	...
	if (current_thread_id == ~0) {
		...
		data_stall_switch_thread();
	}
	else if (last_cycle_data_stalling) {
		data_stall_switch_thread();
	}
	else if (last_cycle_resource_stalling) {
		resource_stall_switch_thread();
	}
	last_cycle_data_stalling = false;
	last_cycle_resource_stalling = false;
...
FREE_INSTR:
	...
	if (instr_info.exec_type == ISA::RISCV::ExecType::CONTROL_FLOW) //SYS is the first non memory instruction type so this divides mem and non mem ops
	{
		...
		if (instr_info.execute_branch(exec_item, instr))
		{
			...
			if (thread._pc == 0x0ull) {
				...
				{
					finished_threads++;
					if (finished_threads == num_threads) simulator->units_executing--;
					current_thread_id = ~0;
				}
				thread_arbiter.remove(thread_id);
	...
	else if (instr_info.exec_type == ISA::RISCV::ExecType::EXECUTABLE)
	{
		...
		{
			last_cycle_resource_stalling = true;
